...
void astPrintNode(ast_t *node)
{
	...
	switch(node->type)
	{
		...
		/* COMMANDS */
		case AST_LIST: 		printf("AST_LIST");				break;
		...
		case AST_THEN: 		printf("AST_THEN");				break;
...
{	
	if (root !=0)
	{
	    ...
	    {	
			...
			/* COMMANDS */	
			case AST_LIST:	decompile(root->son[0]);
				if(root->son[1]!=0)
					fprintf(outputAST,";\n");
				decompile(root->son[1]);					
				break;				
			...
			/* DATA TYPE */
			case AST_WORD: 	 
				fprintf(outputAST,"word "); 
				break;
			case AST_BYTE:		 
				fprintf(outputAST,"byte "); 
				break;
			case AST_BOOL: 	 
				fprintf(outputAST,"bool "); 
				break;
            ...
            case AST_LIT_CHAR: 
            case AST_LIT_STRING:	 
				fprintf(outputAST,"%s",root->symbol->text); 
				decompile(root->son[0]); 
				break;		
			...
			/* I/O */
			case AST_IN:		 
				fprintf(outputAST,"input ");
				fprintf(outputAST,"%s",root->symbol->text); 
				decompile(root->son[1]);					
				break;	
			case AST_OUT:		 
				fprintf(outputAST,"output ");
				decompile(root->son[0]);
				break;
			case AST_OUT_ARG:	
			if(root->symbol !=0)
			...
			case AST_IF:		 
				...
				break;
			case AST_THEN:	 
				fprintf(outputAST,"then "); 
				break;
			case AST_ELSE:	 
				fprintf(outputAST,"else "); 
				break;
			case AST_LOOP:	 
				fprintf(outputAST,"loop "); 
				break;				
			...
			case AST_ATRR_LIT:	
				decompile(root->son[0]);
				if(root->son[1])fprintf(outputAST," ");
				decompile(root->son[1]);					
				break;
